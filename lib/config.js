...
const config = {
  ...
  // ---- Admin view ----
  // Password that gates the /admin consultations dashboard. Set ADMIN_PASSWORD
  // in Vercel to enable it; leave unset to keep the admin endpoint disabled.
  admin: {
...
config.features = {
  ...
  adminEnabled: Boolean(config.admin.password && config.storage.url && config.storage.serviceKey),
};
